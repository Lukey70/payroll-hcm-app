(function(){
  ...
  function toast(message){ h('toast', esc(message)); $('toast').classList.add('open'); setTimeout(()=>$('toast').classList.remove('open'), 2200); }
  ...
  function renderAll(){
    renderMetrics(); renderEmployees(); renderChangeCentre(); renderJobSummary(); renderAdditionalEarnings(); renderLeave(); renderAbsenceBalance(); renderPayslip(); renderCertification(); renderAudit(); renderSettings();
  }
  ...
  function renderEmployees(){
    ...
    h('employeesTable', table(['ID','First Name','Last Name','Type','Department','Position','Start','Termination / Contract End','Rate','Status','Actions'], list.map(e=>[
      esc(e.id), esc(e.firstName||''), esc(e.lastName||''), esc(e.type||''), esc(e.department||''), esc(e.position||''), E.fmtPay(e.startDate), e.status==='Terminated'?`${E.fmtPay(e.terminationDate)}<br>${esc(e.terminationReason||'')}`:(e.type==='Fixed Term'&&e.contractEndDate?`${E.fmtPay(e.contractEndDate)}<br>Expected fixed-term end`:''), E.money(e.hourlyRate), badge(e.status||'Active'), `<button class="icon-btn" data-edit-name="${esc(e.id)}" title="Edit name">✏️</button> <button class="icon-btn" data-view-schedule="${esc(e.id)}" title="View schedule">👁️</button>`
    ])));
    ...
    document.querySelectorAll('[data-edit-name]').forEach(b=>b.addEventListener('click',()=>openEditName(b.dataset.editName)));
  ...
  function openEditName(empId){ const e=emp(empId); if(!e) return; modal('Edit Employee Name', `<input id="editNameId" type="hidden" value="${esc(empId)}"><div class="grid form-grid"><div><label>First Name</label><input id="editFirst" value="${esc(e.firstName||'')}"></div><div><label>Last Name</label><input id="editLast" value="${esc(e.lastName||'')}"></div></div>`, `<button id="saveName">Save Name</button>`, true); $('saveName').addEventListener('click',()=>{ e.firstName=v('editFirst').trim(); e.lastName=v('editLast').trim(); e.name=`${e.firstName} ${e.lastName}`.trim(); addJobEvent(e.id,'Name Change',todayIso(),'Employee name updated','employee',e.id); save(); closeModal(); log('Employee name updated'); renderAll(); }); }
  ...
  function openAbsenceCalendar(){ const e=emp(selectedCalendarEmp); const year=E.parseDate(currentCycle().start).getFullYear(); let body=`<p><strong>${esc(E.employeeName(e))}</strong></p><div class="legend"><span class="annual">Annual Leave</span><span class="personal">Personal Leave</span><span class="lsl">Long Service Leave</span><span class="nonrostered">Non Rostered Day</span></div><div class="calendar">`; for(let m=0;m<12;m++){ const first=new Date(year,m,1); const last=new Date(year,m+1,0); body+=`<div class="month"><h4>${first.toLocaleDateString('en-AU',{month:'long'})}</h4><div class="month-grid">${['Sun','Mon','Tue','Wed','Thu','Fri','Sat'].map(d=>`<div class="cal-head">${d}</div>`).join('')}`; for(let i=0;i<first.getDay();i++) body+='<div class="cal-day blank"></div>'; for(let day=1;day<=last.getDate();day++){ const d=E.iso(new Date(year,m,day)); const sched=E.activeSchedule(state,e.id,d); const hrs=Number((sched&&sched.hoursByDay&&sched.hoursByDay[E.parseDate(d).getDay()])||0); const leave=state.leaveBookings.find(l=>l.empId===e.id&&E.between(d,l.startDate,l.endDate)); let cls=hrs<=0?'nonrostered':''; let title=hrs<=0?'Non Rostered Day':''; if(leave){ cls=leave.type==='Annual Leave'?'annual':leave.type==='Personal Leave'?'personal':leave.type==='Long Service Leave'?'lsl':''; title=leave.type; } body+=`<div class="cal-day ${cls}" title="${esc(title)}"><strong>${day}</strong></div>`; } body+='</div></div>'; } body+='</div>'; modal('Absence Calendar', body, '', false); }
  ...
  function renderCertOutput(){ const c=E.cycleById(v('certCycle')||currentCycle().id); const locked=!!state.certifications[String(c.id)]; const lines=(c.id===currentCycle().id?currentResults():state.payslips.filter(p=>Number(p.cycleId)===Number(c.id))); if(!lines.length){ h('certOutput','<p class="small-note">No payslips generated for this pay period.</p>'); return; } h('certOutput', table(['Employee','Position','Gross','Tax','Super','Net','Certify'], lines.map(p=>[esc(p.employeeName),esc(p.position),E.money(p.gross),E.money(p.tax),E.money(p.superAmt),E.money(p.net),`<input type="checkbox" class="certLine" data-id="${esc(p.id)}" ${locked?'checked disabled':''}>`])) + `<div class="divider"></div><div class="grid form-grid"><div><label>Name</label><input id="certName" ${locked?'readonly':''} value="${esc(state.certifications[String(c.id)]?.name||'')}"></div><div><label>Position</label><input id="certPosition" ${locked?'readonly':''} value="${esc(state.certifications[String(c.id)]?.position||'')}"></div></div><p><label><input type="checkbox" id="certDeclaration" ${locked?'checked disabled':''}> I certify to the best of my knowledge, this pay is correct</label></p><button id="saveCert" ${locked?'disabled':''}>Save</button>`); if(!locked) $('saveCert').addEventListener('click',()=>saveCertification(c.id)); }
  ...
  function renderSettings(){ h('settings', `<h2>Settings</h2><p><strong>Current app version:</strong> v${APP_VERSION}</p><div class="controls"><button id="checkUpdates">Check for Updates</button><button id="changeNotes" class="secondary">Change Notes</button><button id="overnight" class="secondary">Check Overnight Processing</button><button id="finalisePay" class="warning">Finalise Pay</button><button id="publicHolidays" class="secondary">View WA Public Holidays</button></div><div id="settingsOutput" class="small-note"></div>`); $('checkUpdates').addEventListener('click',checkForUpdates); $('changeNotes').addEventListener('click',openChangeNotes); $('overnight').addEventListener('click',()=>checkOvernightProcessing(true)); $('finalisePay').addEventListener('click',openFinalisePay); $('publicHolidays').addEventListener('click',openPublicHolidays); }
